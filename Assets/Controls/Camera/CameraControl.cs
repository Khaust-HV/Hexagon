...
public class CameraControl : MonoBehaviour, ICameraMove, ICameraZoom, ICameraSatelliteMovement, ICameraRaycast
{
    ...
    private void OrbitingCamera() {
        switch (_cameraState) {
            ...
            case CameraState.CameraMoveingToDefault:
                Vector3 stepPosition = Vector3.Lerp(transform.position, _defaultPosition, _smoothSpeed);
    ...
    private void OrbitingCameraSetPosition() {
        Vector3 stepPosition = Vector3.Lerp(transform.position, _satellitePosition, _smoothSpeed);
    ...
    public void SetCameraMovementActive(bool isActive) {
        if (isActive) {
            ...
            if (!_isIECameraMoveingActive) {
                Debug.Log("Movement Started"); // FIX IT !
