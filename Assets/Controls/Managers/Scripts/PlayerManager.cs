...
public sealed class PlayerManager : IPlayerManagerInput, IDisposable {
        ...
        switch (_playerState) {
            case PlayerState.FreeMovementOnMap:
                if (_iCameraRaycast.ScreenPositionIntoRayFromCamera(position, RaycastCheckTargetType.CheckUI, out hit)){
                    ...
                    if (_iHexagonTarget.SetHexagonTarget(hit.collider.GetComponent<HexagonController>().HexagonID)){
