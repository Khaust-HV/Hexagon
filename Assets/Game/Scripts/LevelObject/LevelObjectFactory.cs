...
namespace LevelObject {
    public class LevelObjectFactory : ILevelObjectFactory {
        ...
        public List<I> CreateObjects<I>(GameObject prefab, int number, Transform trParentObject, float size = 0f) where I : class {
            ...
            if (objectsList.Count < number) throw new LevelObjectException(LevelObjectErrorType.InvalidObjectsCreation, $"Not all {prefab.name} have been created {objectsList.Count}/{number}");
        ...
        public List<I> CreateRandomObjects<I>(GameObject[] prefabs, int number, Transform trParentObject, float size = 0f) where I : class {
            ...
            if (objectsList.Count < number)throw new LevelObjectException(LevelObjectErrorType.InvalidObjectsCreation, $"Not all random {prefabs[0].name} have been created {objectsList.Count}/{number}");
